...
def constrict_lipids(lipid_files, xy_scale=0.6, z_scale=1.2):
                ...
                for line in f:
                    if line.startswith("ATOM") or line.startswith("HETATM"):
                        ...
                        for protein_dim in protein_dims:
                            lipid_height = lip_z 
                            if int(2 * round(z/2)) not in protein_dim or int(2 * round((z + lipid_height/2)/2)) not in protein_dim or int(2 * round((z - lipid_height/2)/2)) not in protein_dim:
                                continue
